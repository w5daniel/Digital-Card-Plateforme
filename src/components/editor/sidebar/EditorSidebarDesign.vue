<template>
  <div class="pb-4">
    ...
    <div class="px-3 mb-3">
      <p class="text-xs font-medium mb-2" :class="themeStore.darkMode ? 'text-gray-400' : 'text-gray-500'">
        Couleur de fond
      </p>
      <!-- Tabs Uni / Dégradé -->
      <div class="relative flex rounded-lg p-0.5 mb-3 text-xs" :class="themeStore.darkMode ? 'bg-gray-800' : 'bg-gray-100'">
        <div
        ...
        />
        <button @click="bgMode = 'solid'" class="relative z-10 flex-1 py-1 rounded-md font-medium transition-colors" :class="bgMode === 'solid' ? (themeStore.darkMode ? 'text-gray-100' : 'text-gray-800') : (themeStore.darkMode ? 'text-gray-500' : 'text-gray-400')">Uni</button>
        <button @click="bgMode = 'gradient'" class="relative z-10 flex-1 py-1 rounded-md font-medium transition-colors" :class="bgMode === 'gradient' ? (themeStore.darkMode ? 'text-gray-100' : 'text-gray-800') : (themeStore.darkMode ? 'text-gray-500' : 'text-gray-400')">Dégradé</button>
      </div>

      <!-- Mode Uni -->
      <template v-if="bgMode === 'solid'">
        <div class="flex items-center gap-2 mb-2">
          <label class="relative w-8 h-8 rounded-lg overflow-hidden border-2 cursor-pointer flex-shrink-0 transition-all hover:scale-105" :class="themeStore.darkMode ? 'border-gray-600' : 'border-gray-200'" :style="{ background: editorStore.currentBackground }">
          ...
          <input type="text" :value="editorStore.currentBackground?.startsWith('linear-gradient') ? '' : (editorStore.currentBackground || '')" @change="editorStore.setBackground($event.target.value)" class="flex-1 text-xs px-2 py-1.5 rounded border font-mono outline-none" :class="themeStore.darkMode ? 'bg-gray-800 border-gray-700 text-gray-200' : 'bg-white border-gray-200 text-gray-800'" placeholder="#FFFFFF" />
        </div>
        <div class="grid grid-cols-8 gap-1 mb-3">
          <button v-for="c in bgPresets" :key="c" @click="editorStore.setBackground(c)" class="aspect-square rounded border-2 transition-all hover:scale-110" :style="{ backgroundColor: c }" :class="editorStore.currentBackground === c ? 'border-violet-500 scale-110' : themeStore.darkMode ? 'border-gray-700' : 'border-gray-200'" />
        </div>
      </template>
      ...
      <template v-else>
        <div class="mb-2">
          ...
          <div class="grid grid-cols-4 gap-1">
            <button v-for="dir in [{ a: 0, icon: '↑' }, { a: 45, icon: '↗' }, { a: 90, icon: '→' }, { a: 135, icon: '↘' }, { a: 180, icon: '↓' }, { a: 225, icon: '↙' }, { a: 270, icon: '←' }, { a: 315, icon: '↖' }]" :key="dir.a" @click="bgGradAngle = dir.a; applyBgGradient()" class="py-1 rounded text-sm font-bold transition-all" :class="[bgGradAngle === dir.a ? 'bg-violet-500 text-white' : themeStore.darkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200']">{{ dir.icon }}</button>
          </div>
        </div>
        <div class="flex gap-2 mb-2">
          <div class="flex-1">
            <p class="text-[10px] mb-1" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Début</p>
            <label class="relative w-full h-7 rounded border overflow-hidden cursor-pointer block" :class="themeStore.darkMode ? 'border-gray-600' : 'border-gray-200'" :style="{ background: bgGradFrom }">
          ...
          <div class="flex-1">
            ...
            <label class="relative w-full h-7 rounded border overflow-hidden cursor-pointer block" :class="themeStore.darkMode ? 'border-gray-600' : 'border-gray-200'" :style="{ background: bgGradTo }">
        ...
        <div class="w-full h-6 rounded mb-2 border" :class="themeStore.darkMode ? 'border-gray-700' : 'border-gray-200'" :style="{ background: buildGradientCss(bgGradAngle, bgGradFrom, bgGradTo) }" />
        <div class="flex flex-wrap gap-1.5">
          <button v-for="preset in BG_GRAD_PRESETS" :key="preset.label" @click="applyBgGradPreset(preset)" class="w-7 h-7 rounded border-2 transition-all hover:scale-110 hover:shadow-sm" :style="{ background: buildGradientCss(preset.angle, preset.from, preset.to) }" :class="themeStore.darkMode ? 'border-gray-700 hover:border-violet-500' : 'border-gray-200 hover:border-violet-400'" :title="preset.label" />
        </div>
      </template>
    </div>
    ...
    <div class="px-3 mt-1 mb-3">
      <p class="text-xs font-medium mb-2" :class="themeStore.darkMode ? 'text-gray-400' : 'text-gray-500'">Couleurs du document</p>
      <div class="flex flex-wrap gap-1 items-center">
        <button v-for="color in allDocColors" :key="color" @click="applyDocumentColor(color)" class="w-6 h-6 rounded border-2 transition-all duration-150 hover:scale-110 hover:shadow-sm" :style="{ background: color }" :class="[themeStore.darkMode ? 'border-gray-700 hover:border-gray-500' : 'border-gray-200 hover:border-gray-400', editorStore.selectedIds.length === 0 ? 'opacity-50 cursor-default' : 'cursor-pointer']" :title="editorStore.selectedIds.length > 0 ? `Appliquer aux éléments sélectionnés : ${color}` : color" />
        <label class="w-6 h-6 rounded border-2 border-dashed flex items-center justify-center cursor-pointer transition-all duration-150 hover:scale-110" :class="themeStore.darkMode ? 'border-gray-600 text-gray-500 hover:border-violet-500 hover:text-violet-400' : 'border-gray-300 text-gray-400 hover:border-violet-400 hover:text-violet-500'" title="Ajouter une couleur">
          <Plus class="w-3 h-3" />
          <input type="color" class="hidden" @change="addDocColor($event.target.value)" />
        </label>
        <button @click="showDocGradientPicker = !showDocGradientPicker" class="w-6 h-6 rounded border-2 border-dashed flex items-center justify-center cursor-pointer transition-all duration-150 hover:scale-110 text-[9px] font-bold" :class="showDocGradientPicker ? 'border-violet-500 text-violet-500' : themeStore.darkMode ? 'border-gray-600 text-gray-500 hover:border-violet-500 hover:text-violet-400' : 'border-gray-300 text-gray-400 hover:border-violet-400 hover:text-violet-500'" title="Ajouter un dégradé">▲▼</button>
      </div>
      <div v-if="showDocGradientPicker" class="mt-2 p-2 rounded-lg border" :class="themeStore.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'">
        <div class="flex gap-1.5 mb-2">
          <label class="flex-1 block">
            <span class="text-[9px]" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Du</span>
            <div class="relative h-6 rounded border overflow-hidden mt-0.5" :class="themeStore.darkMode ? 'border-gray-600' : 'border-gray-200'" :style="{ background: docGradFrom }">
          ...
          <label class="flex-1 block">
            <span class="text-[9px]" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Au</span>
            <div class="relative h-6 rounded border overflow-hidden mt-0.5" :class="themeStore.darkMode ? 'border-gray-600' : 'border-gray-200'" :style="{ background: docGradTo }">
        ...
        <button @click="addDocColor(`linear-gradient(135deg, ${docGradFrom} 0%, ${docGradTo} 100%)`); showDocGradientPicker = false" class="w-full text-[10px] py-1 rounded font-medium bg-violet-500 text-white hover:bg-violet-600 transition-colors">Ajouter à la palette</button>
      </div>
      <p v-if="editorStore.selectedIds.length === 0 && allDocColors.length > 0" class="text-[10px] mt-1.5" :class="themeStore.darkMode ? 'text-gray-600' : 'text-gray-400'">Sélectionnez un élément pour appliquer</p>
    </div>
    ...
    <!-- Templates -->
    <div class="px-3">
      <p class="text-xs font-medium mb-2" :class="themeStore.darkMode ? 'text-gray-400' : 'text-gray-500'">Modèles</p>
      <div class="relative mb-2">
        <Search class="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'" />
        <input v-model="designQuery" type="text" placeholder="Rechercher un modèle…" class="w-full text-xs pl-7 pr-7 py-1.5 rounded-lg border outline-none transition-colors" :class="themeStore.darkMode ? 'bg-gray-800 border-gray-700 text-gray-200 placeholder-gray-600 focus:border-violet-500' : 'bg-white border-gray-200 text-gray-800 placeholder-gray-400 focus:border-violet-400'" />
        <button v-if="designQuery" @click="designQuery = ''" class="absolute right-2 top-1/2 -translate-y-1/2" :class="themeStore.darkMode ? 'text-gray-500 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'">
          <X class="w-3.5 h-3.5" />
        </button>
      </div>
      <div class="flex gap-1 mb-2">
        <button v-for="f in designFilters" :key="f.value" @click="designFilter = f.value" class="text-[10px] px-2 py-0.5 rounded-full border transition-colors" :class="[designFilter === f.value ? themeStore.darkMode ? 'bg-violet-900/50 border-violet-500 text-violet-300' : 'bg-violet-50 border-violet-400 text-violet-700' : themeStore.darkMode ? 'border-gray-700 text-gray-400 hover:bg-gray-800' : 'border-gray-200 text-gray-500 hover:bg-gray-50']">
          {{ f.label }} ({{ f.count }})
        </button>
      </div>
      <button @click="startBlankCanvas()" class="w-full flex items-center gap-3 px-3 py-2.5 mb-2 rounded-lg border-2 border-dashed transition-all hover:scale-[1.01]" :class="themeStore.darkMode ? 'border-gray-700 hover:border-violet-500 hover:bg-gray-800' : 'border-gray-300 hover:border-violet-400 hover:bg-violet-50/50'">
        <div class="w-10 h-7 rounded flex items-center justify-center border" :class="themeStore.darkMode ? 'border-gray-600 bg-gray-800' : 'border-gray-200 bg-white'">
          <PlusIcon class="w-4 h-4" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'" />
        </div>
        <div class="text-left">
          <p class="text-xs font-medium" :class="themeStore.darkMode ? 'text-gray-200' : 'text-gray-700'">Carte vierge</p>
          <p class="text-[10px]" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Commencer de zéro</p>
        </div>
      </button>
      <p v-if="filteredDesignTemplates.length === 0 && designQuery" class="text-xs text-center py-4" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">
        Aucun modèle trouvé pour « {{ designQuery }} »
      </p>
      <div class="grid grid-cols-2 gap-2">
        <button v-for="tpl in filteredDesignTemplates" :key="tpl.slug" @click="applyDesignTemplate(tpl)" draggable="true" @dragstart="onTemplateDragStart($event, tpl)" class="relative rounded-lg overflow-hidden border-2 transition-all hover:scale-105 hover:shadow-md group" :class="[themeStore.darkMode ? 'border-gray-700 hover:border-violet-500' : 'border-gray-200 hover:border-violet-400', tpl.isPremium && !authStore.isPremium && !authStore.isAdmin ? 'opacity-60' : '']" :title="tpl.name" style="aspect-ratio: 85.6/54">
