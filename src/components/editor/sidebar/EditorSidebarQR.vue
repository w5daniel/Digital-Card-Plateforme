<template>
  <div class="p-3 flex flex-col gap-4 overflow-y-auto">
    ...
    <div
      ...
      :class="selectedQRElement ? themeStore.darkMode ? 'bg-violet-900/40 text-violet-300' : 'bg-violet-50 text-violet-600' : themeStore.darkMode ? 'bg-gray-800 text-gray-400' : 'bg-gray-50 text-gray-500'"
    >
      <span class="w-1.5 h-1.5 rounded-full shrink-0" :class="selectedQRElement ? 'bg-violet-400 animate-pulse' : 'bg-gray-400'" />
      {{ selectedQRElement ? 'Édition du QR sélectionné' : "Création d'un nouveau QR" }}
    </div>

    <!-- Section: Champs inclus -->
    <div>
      <p class="text-[10px] font-semibold uppercase tracking-wider mb-2" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Champs inclus dans le QR</p>
      <div class="flex flex-col gap-1">
        ...
        >
          <button
            type="button"
            :class="['relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors duration-200', qrConfig.qrFields[key] ? 'bg-violet-500' : themeStore.darkMode ? 'bg-gray-700' : 'bg-gray-300']"
          >
          ...
          <span class="text-xs font-medium w-20 shrink-0" :class="themeStore.darkMode ? 'text-gray-300' : 'text-gray-700'">{{ ROLE_LABELS[key] }}</span>
          <span
            class="text-[10px] truncate flex-1"
            :class="editorStore.contactData[key] ? themeStore.darkMode ? 'text-gray-400' : 'text-gray-500' : themeStore.darkMode ? 'text-gray-600' : 'text-gray-300'"
      ...
      <template v-if="editorStore.contactExtra && editorStore.contactExtra.length">
        <div class="mt-1 pt-2 border-t" :class="themeStore.darkMode ? 'border-gray-800' : 'border-gray-100'">
          <p class="text-[9px] font-semibold uppercase tracking-wider mb-1 px-2" :class="themeStore.darkMode ? 'text-gray-600' : 'text-gray-400'">Champs personnalisés</p>
          <label
          ...
          >
            <button
              type="button"
              :class="['relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors duration-200', qrConfig.qrFields[item.id] ? 'bg-violet-500' : themeStore.darkMode ? 'bg-gray-700' : 'bg-gray-300']"
            >
            ...
            <span class="text-xs font-medium w-20 shrink-0 truncate" :class="themeStore.darkMode ? 'text-gray-300' : 'text-gray-700'">{{ item.label }}</span>
            <span
              class="text-[10px] truncate flex-1"
              :class="item.value ? themeStore.darkMode ? 'text-gray-400' : 'text-gray-500' : themeStore.darkMode ? 'text-gray-600' : 'text-gray-300'"
            >{{ item.value || '—' }}</span>
          </label>
        </div>
      </template>
    ...
    <!-- Section: Mode -->
    <div>
      <p class="text-[10px] font-semibold uppercase tracking-wider mb-2" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Apparence</p>
      <div class="grid grid-cols-2 gap-2">
        <button
          ...
          :class="[qrConfig.qrMode === mode.value ? 'border-violet-500 bg-violet-500/10 text-violet-500' : themeStore.darkMode ? 'border-gray-700 text-gray-400 hover:border-gray-600' : 'border-gray-200 text-gray-500 hover:border-gray-300', mode.value === 'styled' && !authStore.isPremium && !authStore.isAdmin ? 'opacity-60 cursor-not-allowed' : '']"
        >
    ...
    <!-- Section: Couleurs -->
    <div>
      <p class="text-[10px] font-semibold uppercase tracking-wider mb-2" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Couleurs</p>
    ...
    <template v-if="qrConfig.qrMode === 'styled'">
      <div>
        <p class="text-[10px] font-semibold uppercase tracking-wider mb-2" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Style des points</p>
        <div class="grid grid-cols-3 gap-1">
          <button
            ...
            :class="qrConfig.qrDotStyle === s.value ? 'border-violet-500 bg-violet-500/10 text-violet-500 font-semibold' : themeStore.darkMode ? 'border-gray-700 text-gray-400 hover:border-gray-500' : 'border-gray-200 text-gray-500 hover:border-gray-300'"
          >{{ s.label }}</button>
        </div>
      </div>

      <div>
        <p class="text-[10px] font-semibold uppercase tracking-wider mb-2" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Coin externe</p>
        <div class="grid grid-cols-3 gap-1">
          <button
            ...
            class="text-[10px] px-1 py-1.5 rounded-lg border transition-colors text-center"
            :class="qrConfig.qrCornerSquareStyle === s.value ? 'border-violet-500 bg-violet-500/10 text-violet-500 font-semibold' : themeStore.darkMode ? 'border-gray-700 text-gray-400 hover:border-gray-500' : 'border-gray-200 text-gray-500 hover:border-gray-300'"
          >{{ s.label }}</button>
        </div>
      </div>

      <div>
        <p class="text-[10px] font-semibold uppercase tracking-wider mb-2" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Coin interne</p>
        <div class="grid grid-cols-3 gap-1">
          <button
            ...
            class="text-[10px] px-1 py-1.5 rounded-lg border transition-colors text-center"
            :class="qrConfig.qrCornerDotStyle === s.value ? 'border-violet-500 bg-violet-500/10 text-violet-500 font-semibold' : themeStore.darkMode ? 'border-gray-700 text-gray-400 hover:border-gray-500' : 'border-gray-200 text-gray-500 hover:border-gray-300'"
          >{{ s.label }}</button>
        </div>
      </div>
      ...
      <!-- Logo -->
      <div>
        <p class="text-[10px] font-semibold uppercase tracking-wider mb-2" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Logo (optionnel)</p>
        ...
        <div v-if="qrConfig.qrLogoSrc" class="flex items-center gap-2 mb-2">
          <img :src="qrConfig.qrLogoSrc" class="w-10 h-10 object-contain rounded border" :class="themeStore.darkMode ? 'border-gray-600' : 'border-gray-200'" />
        ...
        <button
          ...
          :class="themeStore.darkMode ? 'border-gray-600 text-gray-400 hover:border-violet-500 hover:text-violet-400' : 'border-gray-300 text-gray-500 hover:border-violet-400 hover:text-violet-500'"
    ...
    <div>
      <div class="flex items-center justify-between mb-1">
        <p class="text-[10px] font-semibold uppercase tracking-wider" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Marge</p>
        <span class="text-[10px]" :class="themeStore.darkMode ? 'text-gray-400' : 'text-gray-500'">{{ qrConfig.qrMargin }}px</span>
      </div>
    ...
    <div>
      <p class="text-[10px] font-semibold uppercase tracking-wider mb-2" :class="themeStore.darkMode ? 'text-gray-500' : 'text-gray-400'">Correction d'erreur</p>
      <div class="grid grid-cols-4 gap-1">
        <button
          ...
          :class="[qrConfig.qrErrorCorrection === lvl.value ? 'border-violet-500 bg-violet-500/10 text-violet-500' : themeStore.darkMode ? 'border-gray-700 text-gray-400 hover:border-gray-500' : 'border-gray-200 text-gray-500 hover:border-gray-300', lvl.value !== 'L' && !authStore.isPremium && !authStore.isAdmin ? 'opacity-50 cursor-not-allowed' : '']"
        >
      ...
      <p class="text-[9px] mt-1" :class="themeStore.darkMode ? 'text-gray-600' : 'text-gray-400'">Niveau H recommandé si vous ajoutez un logo.</p>
    </div>
    ...
    <div
      ...
      :class="themeStore.darkMode ? 'border-violet-700 bg-violet-900/30 text-violet-300' : 'border-violet-200 bg-violet-50 text-violet-600'"
    >
