        ...
        >
          ...
          <div class="flex items-start gap-4 mb-4">
            ...
            <div class="flex-1 min-w-0">
              <h3 class="font-bold text-onyx-900 dark:text-white text-base leading-snug">
                {{ title }}
              </h3>
              <p
                v-if="message"
                class="text-sm text-onyx-500 dark:text-onyx-400 mt-1 leading-relaxed"
              >
                {{ message }}
              </p>
            </div>
            <button
              @click="$emit('cancel')"
              class="w-7 h-7 flex items-center justify-center rounded-lg text-onyx-400 hover:bg-powder-100 dark:hover:bg-onyx-700 transition-colors shrink-0"
            >
              <X class="w-4 h-4" />
            </button>
          </div>
          ...
          <div class="flex gap-2 justify-end mt-5">
            <button
              @click="$emit('cancel')"
              class="px-4 py-2 text-sm font-medium rounded-lg text-onyx-600 dark:text-onyx-300 bg-powder-100 dark:bg-onyx-700 hover:bg-powder-200 dark:hover:bg-onyx-600 transition-colors"
            >
              {{ cancelLabel }}
            </button>
            <button
...
const iconBg = computed(() => ({
  ...
  'bg-onyx-100 dark:bg-onyx-700': props.variant === 'logout',
}))
...
const iconColor = computed(() => ({
  ...
  'text-onyx-500 dark:text-onyx-300': props.variant === 'logout',
}))
...
const confirmBtnClass = computed(() => ({
  ...
  'bg-onyx-700 hover:bg-onyx-800 dark:bg-onyx-600 dark:hover:bg-onyx-500':
