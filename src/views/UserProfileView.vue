              ...
              <div class="flex items-center gap-4 mb-6 pb-6 border-b border-powder-100 dark:border-onyx-800">
                <div class="relative group shrink-0">
                  ...
                  <label class="relative block cursor-pointer">
                    ...
                    <div class="absolute inset-0 rounded-full bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 cursor-pointer flex items-center justify-center">
            ...
            <div class="bg-powder-50 dark:bg-onyx-900 border border-powder-200 dark:border-onyx-800 rounded-2xl shadow-sm p-6">
              ...
              <form @submit.prevent="changePassword" class="space-y-4">
                <div v-for="field in passwordFields" :key="field.key">
                  ...
                  <p
                    v-if="field.key === 'confirm' && passwordForm.confirm && passwordForm.new !== passwordForm.confirm"
          ...
          <template v-if="activeTab === 'preferences'">
            ...
            <div class="bg-powder-50 dark:bg-onyx-900 border border-powder-200 dark:border-onyx-800 rounded-2xl shadow-sm p-6">
              ...
              <div class="divide-y divide-powder-100 dark:divide-onyx-800">
                ...
                >
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg flex items-center justify-center" :class="pref.bgClass">
                    ...
                    <tr v-for="inv in mockInvoices" :key="inv.id" class="group">
                      ...
                      <td class="py-3.5">
                        <span
                          ...
                          :class="inv.status === 'Payé'
                            ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400'
                            : 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400'"
                        >
                          <span class="w-1.5 h-1.5 rounded-full" :class="inv.status === 'Payé' ? 'bg-emerald-500' : 'bg-red-500'" />
                          {{ inv.status }}
                        </span>
                      </td>
...
import {
  User, Lock, Sliders, Star, CreditCard, Camera,
  Bell, Moon, Mail, QrCode, Eye, EyeOff,
  Save, Download, LogOut, CheckCircle, AlertTriangle,
  Shield, Monitor, Loader2, Receipt,
...
const confirmDeleteAccount = () => {
  if (confirm('Supprimer votre compte ? Cette action est irréversible et supprimera toutes vos cartes.')) {
    authStore.logout()
    router.push('/')
  }
}
...
const confirmCancelSubscription = () => {
  if (confirm("Annuler votre abonnement Premium ? Vous conserverez vos avantages jusqu'à la fin de la période en cours.")) {
    if (authStore.user) {
...
const formattedLoginDate = new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })
